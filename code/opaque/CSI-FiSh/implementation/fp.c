...
static void reduce_once(uint_s *x)
{
    uint_s t;
    if (!uint_sub3(&t, x, &p))
        *x = t;
...
{
    uint_add3((uint_s *) x, (uint_s *) y, (uint_s *) z);
    reduce_once((uint_s *) x);
}
...
{
    if (uint_sub3((uint_s *) x, (uint_s *) y, (uint_s *) z))
        uint_add3((uint_s *) x, (uint_s *) x, &p);
}
...
void fp_enc(fp *x, uint_s const *y)
{
    fp_mul3(x, (fp *) y, &r_squared_mod_p);
}

void fp_dec(uint_s *x, fp const *y)
{
...
static void fp_pow(fp *x, uint_s const *e)
{
    fp y = *x;
    *x = fp_1;
    for (size_t k = 0; k < LIMBS; ++k) {
