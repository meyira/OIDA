...
//const private_key **_server_keys=&server_keys;
void init(){
  //&server_keys=(private_key*)calloc(sizeof(private_key), HASHLEN+1);
...
//
  ...
  while(1){
    ...
    pthread_t tid;
    // uint8_t c_id=id; 
    if (pthread_create(&tid, NULL, evaluate,(void*)csocket)!=0){
