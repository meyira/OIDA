...
namespace OpusPsi{
    ...
    void BitVector::append(const block &b)
    {
        auto destIdx = mNumBits / 8;
        resize(mNumBits + 128);
        memcpy(mData + destIdx, (uint8_t*)&(b), 16);
    }
    ...
    {
        if (idx >= mNumBits || idx % 8 != 0) throw std::runtime_error("rt error at ");
        return mData[(idx / 8)];
    }

    BitReference BitVector::operator[](const uint64_t idx) const
    {
        if (idx >= mNumBits) throw std::runtime_error("rt error at ");
...
//    }
    ...
    void BitVector::operator^=(const BitVector& A)
    {
        if (mNumBits != A.mNumBits) throw std::runtime_error("rt error at " );
        for (uint64_t i = 0; i < sizeBytes(); i++)
        {
            mData[i] ^= A.mData[i];
        }
