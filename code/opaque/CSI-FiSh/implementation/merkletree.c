...
void hash_up(const uint_s *data, unsigned char *indices, const unsigned char *in, int in_len, unsigned char *root, const unsigned char * merkle_key){
	unsigned char *tree = calloc(1,((2<<PK_TREE_DEPTH)-1)*SEED_BYTES);
	uint16_t buf[9+sizeof(uint_s)+SEED_BYTES];
	uint16_t *key = buf;
	memcpy(buf+1,merkle_key,SEED_BYTES);
	unsigned char *input = (unsigned char *)(buf+9);
	*key = 0;
	...
	for(i = ((2<<PK_TREE_DEPTH) -2) ; i>=((1<<PK_TREE_DEPTH) -1) ; i--){
		if(class_tree[i] == 1){
			memcpy(input,(unsigned char *)(data+ (i-((1<<PK_TREE_DEPTH) -1))),sizeof(uint_s));
			*key = i; 
			TREEHASH((unsigned char *) buf, sizeof(uint_s)+ 9*sizeof(uint16_t), tree + i*SEED_BYTES);
		} 
