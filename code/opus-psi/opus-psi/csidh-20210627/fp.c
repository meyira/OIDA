...
static void reduce_once(limbs *x)
{
    limbs t;
    if (!uint_sub3(&t, x, &p))
        *x = t;
...
{
    bool c = uint_add3((limbs *) x, (limbs *) y, (limbs *) z);
    (void) c; assert(!c);
    reduce_once((limbs *) x);
}
...
{
    if (uint_sub3((limbs *) x, (limbs *) y, (limbs *) z))
        uint_add3((limbs *) x, (limbs *) x, &p);
}
