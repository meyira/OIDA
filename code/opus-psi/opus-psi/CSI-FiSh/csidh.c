...
void priv_print_add(private_key const *x, private_key const *y)
{
  for(size_t i=0; i<NUM_PRIMES; ++i)
    printf(" %d ", x->e[i]+ y->e[i]);
  puts("");
}
void priv_print_sub(private_key const *x, private_key const *y)
{
  for(size_t i=0; i<NUM_PRIMES; ++i)
    printf(" %d ", x->e[i]- y->e[i]);
  puts("");
...
static bool validate_rec(proj *P, proj const *A, size_t lower, size_t upper, uint_s *order, bool *is_supersingular)
{
    assert(lower < upper);
...
void large_action(public_key *out, public_key const *in, large_private_key const *priv)
{
    ...
    uint32_t e[2][NUM_PRIMES];

    for (size_t i = 0; i < NUM_PRIMES; ++i) {

        int32_t t = (int32_t) priv->e[i] ;
