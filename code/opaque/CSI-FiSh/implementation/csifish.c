...
void csifish_keygen(unsigned char *pk, unsigned char *sk){
	...
	#ifdef MERKLEIZE_PK
		uint_s *curves = malloc(sizeof(uint_s[PKS]));
	#else
		uint_s* curves = (uint_s*) PK_CURVES(pk);
	#endif
	...
	for (int i = 0; i < PKS; ++i)
	{
		...
		fp_dec(&curves[i], (fp *) &out.A);
	}

	#ifdef MERKLEIZE_PK
		// build merkle tree on all the curves
		build_tree((unsigned char *) curves, sizeof(uint_s), PK_TREE_DEPTH, SK_TREE(sk),SK_MERKLE_KEY(sk));
...
void csifish_sign(const unsigned char *sk,const unsigned char *m, uint64_t mlen, unsigned char *sig, uint64_t *sig_len){
	...
	for(int k=0 ; k<ROUNDS; k++){
		...
		// convert to uint64_t's
		fp_dec(&curves[k], (fp *) &out.A);
	}

	#ifdef MERKLEIZE_PK
		// copy curves to signature
		memcpy(SIG_CURVES(sig),(unsigned char *) curves, sizeof(uint_s[ROUNDS]));
	#endif

	// hash curves
	unsigned char curve_hash[HASH_BYTES];
	HASH((unsigned char *) curves, sizeof(uint_s[ROUNDS]), curve_hash);
...
int csifish_verify(const unsigned char *pk, const unsigned char *m, uint64_t mlen, const unsigned char *sig, uint64_t sig_len){
	...
	// hash curves
	uint_s* curves = (uint_s*) SIG_CURVES(sig);
	unsigned char curve_hash[HASH_BYTES];
	HASH((unsigned char *) curves, sizeof(uint_s[ROUNDS]), curve_hash);
	...
	uint_s *pkcurves = malloc(sizeof(uint_s[PKS])); 
	unsigned char *indices = calloc(1,PKS);
	for(int i=0; i<ROUNDS; i++){
		...
		else if( memcmp(&pkcurves[challenges_index[i]],&pkcurve,sizeof(uint_s)) ){
...
int csifish_verify(const unsigned char *pk, const unsigned char *m, uint64_t mlen, const unsigned char *sig, uint64_t sig_len){
	...
	uint_s  curves[ROUNDS];
	uint_s* pkcurves = (uint_s*) PK_CURVES(pk); 
	for(int i=0; i<ROUNDS; i++){
		// encode starting point
		public_key start,end;
		fp_enc((fp *) &(start.A), &pkcurves[challenges_index[i]]);

		if(challenges_sign[i]){
			fp_mul2((fp *) &start.A,&minus_one);
		}
